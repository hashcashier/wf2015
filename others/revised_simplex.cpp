...
// row 1:m lessthan inequalities
// column 0:n1 inequality coefficients// column n inequality constants (0 for obj. function)
...
double simplex(int m, int n, double c[][maxn], double x[]) {
	...
	for(;;) {
		...
		for (i=1,ii=0;i<=m;i++)
			if ((ine[i][jj]>eps) && (!ii || (ine[i][n]/ine[i][jj] < ine[ii][n]/ine[ii][jj]-eps)
			||	((ine[i][n]/ine[i][jj] < ine[ii][n]/ine[ii][jj]+eps) && (basis[i] < basis[ii]))))
				ii=i;
		if (ine[ii][jj] <= eps) return inf;
		pivot(m,n,ii,jj);
	}
